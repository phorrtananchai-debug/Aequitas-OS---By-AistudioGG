...
import { calculateLayerStats, calculatePortfolioDrift, safeToLocaleString } from '../core/utils';
...
export default function DailyBriefTab({ portfolioValue, dailyBrief, holdings }: DailyBriefProps) {
  ...
  return (
    <div className="space-y-8 animate-fade-in text-slate-800 dark:text-[#F4EEE4] pb-12 font-sans">
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-start pb-4 border-b border-slate-100 dark:border-slate-800/45">
              ...
              <div className="text-right">
                <span className="text-3xl font-black text-slate-900 dark:text-white">${safeToLocaleString(portfolioValue, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100/70 dark:bg-slate-900/10 dark:border-slate-800/10 space-y-2">
                ...
                <div className="space-y-2 font-sans">
                  {(layerStats?.layers || []).map(layer => (
                    <div key={layer.name} className="flex justify-between items-center text-xs">
                      <span className="text-slate-500 font-medium">{layer.name}</span>
                      <span className="font-mono font-bold text-slate-800 dark:text-slate-350">{layer.pct ?? 0}%</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100/70 dark:bg-slate-900/10 dark:border-slate-800/10 space-y-2">
                ...
                <div className="space-y-2 font-sans">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="font-mono font-bold text-slate-800 dark:text-slate-350">{drift}%</span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
        ...
        <div className="lg:col-span-4 glass-panel rounded-3xl p-6 shadow-sm flex flex-col justify-between border border-slate-205/60 dark:border-slate-800/25 bg-white text-slate-800">
          <div className="space-y-6">
            ...
            <div className="p-3.5 rounded-xl bg-amber-50/40 border border-amber-105/45 dark:bg-amber-950/10 dark:border-amber-808/20 shadow-sm flex items-start gap-2.5">
              <div className="p-1 rounded bg-amber-100 text-amber-600 shrink-0 mt-0.5 animate-pulse">💰</div>
              <div>
      ...
      <section className="glass-panel rounded-3xl p-6 sm:p-8 border border-slate-205/60 dark:border-slate-800/25 shadow-sm bg-white text-slate-800">
        ...
        <div className="p-5 rounded-2xl bg-slate-50 border border-slate-100 dark:bg-slate-950/20 dark:border-slate-900/40 shadow-sm leading-relaxed text-sm text-slate-650 dark:text-slate-300 font-sans space-y-4">
          <div className="flex gap-2 items-start">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">{dailyBrief?.aiObservation || 'No active guidance suggested.'}</p>
            </div>
          </div>
          ...
          <div className="flex gap-2 items-start pb-2">
            ...
            <div>
              ...
              <ul className="text-xs text-slate-500 mt-1 list-disc pl-4 space-y-1">
                {(dailyBrief?.whatToReviewToday || []).length > 0 ? dailyBrief.whatToReviewToday.map((act, idx) => (
                  <li key={idx}>{act}</li>
                )) : (
                  <li>No review items scheduled for today.</li>
                )}
              </ul>
            </div>
          </div>
